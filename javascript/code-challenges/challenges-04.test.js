...
const updateAnimal = (arr, callback) => {
  // Solution code here...
  const names = arr.map(element => callback(element))
  return names
};
...
const sortByPrice = (arr) => {
  // Solution code here...
  const sorted = arr.sort((element1, element2) => {
...
const alphabetizeBetter = (arr) => {
  ...
  const filetAlpha = arr.sort((element1, element2) => {

    if (element1.toLowerCase() < element2.toLowerCase()) return -1;
    if (element1.toLowerCase() > element2.toLowerCase()) return 1;
    return 0;
  })
