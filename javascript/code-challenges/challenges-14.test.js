...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const biggerThan = arr.reduce((name, massValue) => {
    if (parseInt(massValue.mass) > parseInt(arr[0].mass)) {
      if (name) {
        name += ' - ';
      }
      name += massValue.name;
...
const sortBy = (property, arr) => {
  // Solution code here...
  const sorted = arr.sort((value, value2) => {
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  // it's hard i tried alot i checked the code from the internt i knew the answer but i did not solve it alone =) 
  for (let i = 0; i < board.length; i++) {
    for (let j = 0; j < board.length; j++) {
      ...
      if (board[i][j] === board[i + 1][j + 1] && board[i + 1][j + 1] === board[i + 2][j + 2] || board[i + 2][j + 2] === board[i + 1][j + 1] && board[i + 1][j + 1] === board[i][j] && board[i][j] !== '') {
        return true
      }
    }
    return false
