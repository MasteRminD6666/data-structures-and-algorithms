...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach(element => {
    ...
    powArr.push(Math.pow(element, 3))
  });
  return powArr
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let item = 0; item < arr.length; item++) {
    newArr.push(Math.pow(2, arr[item]))
...
// i know i should use a map downbelow
 newArray.forEach(item =>{
   secArr.push(item.ability.name)
 })
  return secArr;
};
